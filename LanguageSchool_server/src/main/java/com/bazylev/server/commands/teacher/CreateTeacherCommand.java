...
public class CreateTeacherCommand implements Command {
    ...
    public CreateTeacherCommand(TeacherService teacherService) {
    ...
    public Response execute(Request request, Session session) {
        if (!session.hasAnyRole(Role.ADMIN)) {
            return Response.forbidden("Только администратор может добавлять преподавателей");
        }
        return teacherService.createTeacher(request.getData());
    }
}

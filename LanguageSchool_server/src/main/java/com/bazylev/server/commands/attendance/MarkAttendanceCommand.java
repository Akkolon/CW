...
public class MarkAttendanceCommand implements Command {

    private final AttendanceService attendanceService;

    public MarkAttendanceCommand(AttendanceService attendanceService) {
    ...
    public Response execute(Request request, Session session) {
        if (!session.hasAnyRole(Role.ADMIN, Role.TEACHER)) {
            return Response.forbidden("Только преподаватель или администратор может отмечать посещаемость");
        }
        return attendanceService.markAttendance(request.getData());
    }
}

...
public class GetStudentHistoryCommand implements Command {

    private final StudentService studentService;

    public GetStudentHistoryCommand(StudentService studentService) {
    ...
    public Response execute(Request request, Session session) {
        if (!session.hasAnyRole(Role.ADMIN, Role.TEACHER)) {
            return Response.forbidden("Недостаточно прав для просмотра истории студента");
        }
        return studentService.getStudentHistory(request.getData());
    }
}

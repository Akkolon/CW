...
public class StudentEditFormController {
    ...
    public void setStudent(Student student) {
        ...
        activeCheck.setSelected(student.isActive());
        loadPersonData(student.getPersonId());
    }
    ...
    private void onSave() {
        ...
        data.addProperty("id",         editingStudent.getId());
        ...
        Response resp = ServerConnection.getInstance()
                .send(RequestBuilder.ofRaw(RequestType.UPDATE_STUDENT, data.toString()));

        if (resp.getStatus() == ResponseStatus.OK) {
            AlertUtil.showInfo("Успех", "Данные студента обновлены");
